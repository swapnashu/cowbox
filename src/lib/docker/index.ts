...
/**
 * Ensure the Traefik reverse proxy container is running on port 80/443
 */
export async function ensureTraefikRunning(letsEncryptEmail = ""): Promise<string> {
  ...
  if (existingTraefik) {
    ...
    if (existingTraefik.State !== "running") {
      await containerObj.start().catch(() => {});
    }
    return existingTraefik.Id;
  }
